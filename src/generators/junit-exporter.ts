...
export function exportJunitXml(
  ...
  xml += `<testsuites name="Evoveo Smart Reporter" tests="${totalTests}" failures="${totalFailures}" errors="${totalErrors}" skipped="${totalSkipped}" time="${totalTime.toFixed(3)}">\n`;
  ...
  for (const [file, tests] of suites) {
    ...
    for (const test of tests) {
      ...
      // Evoveo Smart Reporter custom properties
      xml += `      <properties>\n`;
