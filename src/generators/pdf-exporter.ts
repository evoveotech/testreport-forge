 ...
 * @param options - Evoveo Smart Reporter options (unused currently, reserved for future PDF config)
...
export async function exportPdfReport(
  ...
  if (!fs.existsSync(htmlPath)) {
    console.warn('Evoveo Smart Reporter: HTML report not found. Skipping PDF generation.');
    return null;
  }
  ...
  try {
    ...
    console.warn('Evoveo Smart Reporter: PDF export requires playwright-core. Skipping PDF generation.');
    return null;
  }
  ...
  try {
    ...
    console.warn('Evoveo Smart Reporter: PDF generation failed:', err instanceof Error ? err.message : 'Unknown error');
    return null;
  } finally {
    if (browser) {
      await browser.close();
