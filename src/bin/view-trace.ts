...
function printUsage(): void {
  console.log(`
Usage: evoveo-smart-reporter-view-trace <trace.zip> [options]
...
Examples:
  evoveo-smart-reporter-view-trace ./traces/my-trace.zip
  evoveo-smart-reporter-view-trace ./traces/my-trace.zip --dir ./example
`);
}
...
async function main(): Promise<void> {
  ...
  if (!fs.existsSync(traceFsPath) || !fs.statSync(traceFsPath).isFile()) {
    ...
    console.error(`Example: npx evoveo-smart-reporter-view-trace ${JSON.stringify(options.tracePath)} --dir "./example"`);
    process.exit(1);
  }
