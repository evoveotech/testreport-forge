...
```
evoveo-smart-reporter/          [Monorepo root]
...
This will:
1. Run pytest tests
2. Convert JSON to Evoveo Smart Reporter format
...
```python
# conftest.py
pytest_plugins = ["evoveo_smart_reporter"]
```
...
1. Edit files in `python/evoveo_smart_reporter_python/`
...
3. Check Python package is installed: `pip show evoveo-smart-reporter-python`
...
2. **Python**: Update `python/evoveo_smart_reporter_python/converter.py`
...
- Python: python/evoveo_smart_reporter_python/converter.py (add field to conversion)
```
