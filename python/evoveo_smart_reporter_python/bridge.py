"""
Bridge to call the Node.js Evoveo Smart Reporter from Python.
"""
import json
import subprocess
import sys
...
def _get_dist_root() -> Path:
    ...
    raise RuntimeError(
        "Cannot find the compiled Evoveo Smart Reporter JS files.\n"
...
def _find_monorepo_root() -> Optional[Path]:
    """
    Walk upward looking for the evoveo-smart-reporter package.json.

    Returns the repo root Path or None.
    """
    # This file: python/evoveo_smart_reporter_python/bridge.py
...
def _is_valid_root(p: Path) -> bool:
    ...
    try:
        ...
        return data.get("name") == "evoveo-smart-reporter"
    except Exception:
        return False
...
class SmartReporterBridge:
    ...
    This class handles:
    1. Converting pytest JSON to Evoveo Smart Reporter format
    ...
    def generate_report(
        ...
        """
        # Convert pytest JSON to Evoveo Smart Reporter format
