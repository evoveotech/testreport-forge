...
goal_template: "Stop iterating when the task meets all acceptance criteria for: brutal-honest"
...
```
Goal     -> Apply brutal-honest to achieve the task objective
...
**Recursive Goal:** Stop iterating when the task meets all acceptance criteria for: brutal-honest
...
- Job: Research everything the Evoveo Tech Agent cannot know from training data:
  ...
  - **Domain-specific patterns** — best practices the Evoveo Tech Agent might not know
  - **Anything uncertain** — if the Evoveo Tech Agent encounters something it's not confident about, it searches
...
Spawn one SubAgent per selected review domain using the Evoveo Tech Agent tool. Only spawn agents for domains the user selected in Phase 1 Step 1. Launch ALL selected agents in parallel (single message with multiple Agent tool calls).
...
Use this template when dispatching each agent via the Evoveo Tech Agent tool:
...
Include in the Evoveo Tech Agent prompt:
...
- **Evoveo Tech Agent-generated aesthetic** — Generic design with no brand identity: purple gradients, single sans-serif font, three-column icon grids, uniform rounded corners, 0.1 opacity shadows on everything (web projects)
