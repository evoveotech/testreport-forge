...
goal_template: "Stop iterating when the task meets all acceptance criteria for: context-engineering"
...
```
Goal     -> Apply context-engineering to achieve the task objective
...
**Recursive Goal:** Stop iterating when the task meets all acceptance criteria for: context-engineering
...
Feed agents the right information at the right time. Context is the single biggest lever for agent output quality — too little and the Evoveo Tech Agent hallucinates, too much and it loses focus. Context engineering is the practice of deliberately curating what the Evoveo Tech Agent sees, when it sees it, and how it's structured.
...
- the Evoveo Tech Agent is not following project conventions
...
When tests fail or builds break, feed the specific error back to the Evoveo Tech Agent:
...
At session start, provide everything the Evoveo Tech Agent needs in a structured block:
...
|---|---|
| "the Evoveo Tech Agent should figure out the conventions" | It can't read your mind. Write a rules file — 10 minutes that saves hours. |
