...
goal_template: "Stop iterating when the task meets all acceptance criteria for: browser-testing-with-devtools"
...
```
Goal     -> Apply browser-testing-with-devtools to achieve the task objective
...
**Recursive Goal:** Stop iterating when the task meets all acceptance criteria for: browser-testing-with-devtools
...
Use Chrome DevTools MCP to give your agent eyes into the browser. This bridges the gap between static code analysis and live browser execution — the Evoveo Tech Agent can see what the user sees, inspect the DOM, read console logs, analyze network requests, and capture performance data. Instead of guessing what's happening at runtime, verify it.
...
- Automated UI testing through the Evoveo Tech Agent
...
There is also `--autoConnect` (Chrome 144+, requires enabling remote debugging via `chrome://inspect/#remote-debugging`), which attaches the Evoveo Tech Agent to your **running** Chrome instead. Only use it when the test genuinely needs your logged-in state — see Profile Isolation under Security Boundaries first.
...
- Treat "the Evoveo Tech Agent can see my open tabs" as a finding to surface to the user, not a convenience to exploit.
...
For complex UI issues, write a structured test plan the Evoveo Tech Agent can follow in the browser:
...
| "I'll check the browser manually later" | DevTools MCP lets the Evoveo Tech Agent verify now, in the same session, automatically. |
