...
### Changed
- **Fully open source again.** All licensing, tiers, trials, and paid gating removed — every feature (AI analysis, exports, quality gates, quarantine, themes, branding, notifications, live run controls) is now available to everyone under Apache-2.0.
...
- Report branding defaults to "Evoveo Smart Reporter" (custom `branding.title` still supported).
...
- **Quality Gates**: configurable thresholds (min pass rate, max flaky rate, min stability grade) with CI exit codes via `npx evoveo-smart-reporter gate`
...
- **AI Health Digest**: weekly/daily/monthly trend summaries from history data via `npx evoveo-smart-reporter digest`
...
- **Browser/Project Badges**: display browser badges (chromium, firefox, webkit) and project names for multi-browser/multi-project setups ([#23](https://github.com/evoveotech/testreport-forge/issues/23))
- **Full Annotations Support**: capture and display all test annotations (`@slow`, `@fixme`, `@skip`, custom) with icons and coloured badges
- **Step Filtering**: `filterPwApiSteps` option to hide verbose `pw:api` steps and show only custom `test.step()` entries ([#22](https://github.com/evoveotech/testreport-forge/issues/22))
- **Path Resolution**: `relativeToCwd` option to resolve `outputFile` and `historyFile` relative to CWD instead of Playwright's rootDir ([#20](https://github.com/evoveotech/testreport-forge/issues/20))
- **Project History Isolation**: `projectName` option with `{project}` placeholder in `historyFile` path for per-project history ([#21](https://github.com/evoveotech/testreport-forge/issues/21))
- Cucumber integration documentation (playwright-bdd and cucumber-playwright)

### Fixed

- **RangeError with Large Test Suites** ([#19](https://github.com/evoveotech/testreport-forge/issues/19)): prevent crash when processing suites with many tests
...
- **Retries Double-Counted** ([#17](https://github.com/evoveotech/testreport-forge/issues/17))
...
- **Expected Failures Incorrectly Reported** ([#16](https://github.com/evoveotech/testreport-forge/issues/16))
...
- **Improved Tag Extraction** ([#15](https://github.com/evoveotech/testreport-forge/issues/15))
...
- **Better Console Output** ([#15](https://github.com/evoveotech/testreport-forge/issues/15))
  - Report path now includes helpful commands to open the report
  - Shows `npx playwright show-report` and `open` commands

- **Custom Attachments Support** ([#15](https://github.com/evoveotech/testreport-forge/issues/15))
...
- **Inline Trace Viewer** ([#13](https://github.com/evoveotech/testreport-forge/issues/13))
...
- **Attachment Gallery** ([#14](https://github.com/evoveotech/testreport-forge/issues/14))
...
- **Google Gemini AI Support** ([#18](https://github.com/evoveotech/testreport-forge/pull/18))
...
- **Timed Out Tests Now Counted as Failed** ([#12](https://github.com/evoveotech/testreport-forge/issues/12))
...
This release represents a complete visual overhaul of the Evoveo Smart Reporter, featuring a modern sidebar-based navigation system and numerous UX improvements.
...
- Initial Evoveo Smart Reporter implementation
